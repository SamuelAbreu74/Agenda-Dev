...
export default function PersonComponent() {
    ...
    const handleDeletePerson = async (personId: string) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/persons/${personId}`, {
                method: "DELETE"
            })
            const data = await response.json();
            console.log(data);
            ...
            {persons.map((person: Person) => (
                <div key={person.id} className="flex items-center justify-between px-4 py-3  text-gray-800  hover:bg-gray-100 hover:shadow-blue-950">
                    ...
                    <div className="flex gap-5">
                        ...
                        {/* Botão de Deletar */}
                        <button className="cursor-pointer flex p-1 w-8 gap-2 h-8  rounded-full items-center bg-gray-100 hover:bg-red-200 transition" onClick={() => handleDeletePerson(person.id)}>
