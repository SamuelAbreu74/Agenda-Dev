...
export default function CompanyComponent({ onEdit }: CompanyComponentProps) {
            ...
            {companies.map((company: Company) => (
                <div key={company.id} className="flex items-center justify-between px-4 py-3  text-gray-800  hover:bg-gray-100 hover:shadow-blue-950">
                    ...
                    <div className="flex gap-5">
                        <Link href={`https://wa.me/55${company.whatsappContact}`} className="cursor-pointer flex p-1 min-w-30 gap-2 min-h-8 rounded bg-green-500 hover:bg-green-600 transition-colors"><img className="invert" width="24" height="24" src="https://img.icons8.com/material-outlined/24/whatsapp--v1.png" alt="whatsapp--v1" /> <h1 className="text-white">Whatsapp</h1></Link>
                        {/* Botão de Editar */}
                        <button className="cursor-pointer flex p-1 w-8 gap-2 h-8  rounded-full items-center bg-gray-100 hover:bg-blue-200 transition" onClick={() => onEdit(company)}>
                        ...
                        {/* Botão de Deletar */}
                        <button className="cursor-pointer flex p-1 w-8 gap-2 h-8  rounded-full items-center bg-gray-100 hover:bg-red-200 transition" onClick={() => handleDeleteCompany(company.id)}>
