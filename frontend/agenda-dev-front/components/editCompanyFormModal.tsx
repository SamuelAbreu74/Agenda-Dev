...
export default function EditcompanyFormModal({ isOpen, onClose, company, onUpdate }: EditCompanyFormModalProps) {
    ...
    useEffect(() => {
        if (company && isOpen) {
            setFormData({
                ...
                email: company.email || '',
                cnpj: company.cnpj || '',
                accountable: typeof company.accountable === 'object'
                    ? (company.accountable as any).id
                    : company.accountable || '',
                cellContact: company.cellContact || '',
                whatsappContact: company.whatsappContact || '',
                fixedContact: company.fixedContact || '',
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/companies/${company.id}`, {
                method: "PATCH",
                headers: {
    ...
    const handleLogoUpdate = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const response = await fetch(`http://localhost:3001/companies/${company.id}`, {
            ...
            body: JSON.stringify({ logoUrl: publicUrl }), // Enviando a URL real do Supabase
        });
            ...
            <div className="flex flex-col w-full max-w-3xl max-h-[90vh] overflow-y-auto border-2 rounded-2xl border-black shadow-md shadow-blue-200 p-6 bg-blue-950 text-white">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2">
                        ...
                        <input name="logoUrl" onChange={handleLogoUpdate} type="file" accept="image/*" className="w-full p-1 border rounded text-white file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-sm file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                        {formData.logoUrl && (
